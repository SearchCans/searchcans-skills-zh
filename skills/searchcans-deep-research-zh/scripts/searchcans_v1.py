...
def post(endpoint: str, payload: dict[str, Any], timeout_seconds: int = 35, retries: int = 3) -> dict[str, Any]:
    """POST JSON, retry only documented transient errors, and return the full body."""
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if error.code in FATAL_HTTP_CODES:
                ...
                raise SearchCansError(f"HTTP {error.code}: {detail}") from error
            last_error = error
            if error.code not in RETRYABLE_HTTP_CODES:
...
def normalize_organic(data: Any) -> list[dict[str, Any]]:
    """Accept the documented v1 shape and the legacy-compatible array shape."""
